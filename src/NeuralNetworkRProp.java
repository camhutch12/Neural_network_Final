...
// Cameron Hutchings
 class NeuralNetworkRProp implements Serializable {
    ...
    public NeuralNetworkRProp(List<Float> inputs, int hidden_layers, List<Float> output) {
        this.inputs = inputs;
        this.amount_of_nodes_in_hidden_layer = hidden_layers;
        layers = new ArrayList<>();
        this.output = output;
        setupGraph();
        this.init_rprop_weights();
        ...
        for (Neuron hidden_layer_neuron : this.layers.get(1)) {
            ...
            for (Neuron output_layer_neuron : this.layers.get(2)) {
                if (hidden_layer_neuron.contains(output_layer_neuron.id)) {
                    ...
                    hidden_layer_neuron.delta.add( hidden_layer_neuron.learning_rates_per_weight.get(c) * hidden_layer_neuron.output * output_layer_neuron.error);
                }
                c++;
            }
...
//            for (Neuron hidden_layer_neurons : this.layers.get(1)) {
//                if (input_layer_neurons.contains(hidden_layer_neurons.id)) {
//                    input_layer_neurons.delta.add((Contants.LEARNING_RATE * input_layer_neurons.intput_feature * hidden_layer_neurons.ei));
//
//
//                    //System.out.println(input_layer_neurons.delta);
...
//    }
    ...
    private List feed_foreward() {
        ...
        for (Neuron output : this.layers.get(1)) {
            ...
            output.output = Contants.sigmoid(output.input);
        }
...
//
        ...
        /*System.out.print("The expected ouput is: ");
        for (float expected_output : list_of_lists.get(1)) {
            System.out.print(expected_output);
            System.out.print(" ");
